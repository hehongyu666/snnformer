...
class EfficientFormerV2_RGB_Encoder(nn.Module):
    def __init__(self, layers, embed_dims=None,
                 ...
                 drop_rate=0., drop_path_rate=0.,
                 use_layer_scale=True, layer_scale_init_value=1e-5,
                 fork_feat=True,  #  输出多尺度特征
                 init_cfg=None, pretrained=None,
                 resolution=(260, 346),  #  适配 RGB 分辨率
                 **kwargs):
        super().__init__()
        ...
        for i in range(len(layers)):
            stage = eformer_block(embed_dims[i], i, layers,
                                  ...
                                  use_layer_scale=use_layer_scale,
                                  layer_scale_init_value=layer_scale_init_value,
                                  resolution=(math.ceil(resolution[0] / (2 ** (i + 2))),
                                              math.ceil(resolution[1] / (2 ** (i + 2)))),
                                  )
    ...
    def forward(self, x):
        x = self.patch_embed(x)  #  Patch embedding 适配输入 RGB 图像
...
def build_efficientformer(args):
    model = EfficientFormerV2_RGB_Encoder(
